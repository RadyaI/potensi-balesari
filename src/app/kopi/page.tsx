    ...
    <main className={`${fraunces.variable} flex-1 overflow-x-clip bg-[#F3EDE0] text-[#3A2E22]`}>
      ...
      <header className="navbar-glass fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#2B1A0E]/30 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-6 py-3">
          <Link href="/" className="font-display flex items-center gap-2.5 text-lg font-semibold text-[#F3EDE0]">
      ...
      >
        ...
        <div className="hero-isi relative z-10 mx-auto grid max-w-6xl items-center gap-12 px-6 pt-32 pb-24 sm:pt-36 lg:grid-cols-[1.05fr_0.95fr] lg:pb-32">
          <div>
          ...
          <ul className="mt-10 grid gap-4 sm:grid-cols-2">
            {c.jenis.items.map((j, i) => (
              <Reveal
                ...
                index={i}
                ulang
                className={`relative overflow-hidden rounded-2xl border p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-md motion-reduce:hover:translate-y-0 ${
                  j.sorot ? "border-[#6B4226]/30 bg-[#F8F4EA]" : "border-[#3A2E22]/10 bg-[#F8F4EA]/70"
                }`}
              >
                ...
                <div className="relative flex items-start justify-between gap-3">
                  ...
                  <span
                    className={`shrink-0 rounded-full px-3 py-1 text-[10px] font-bold tracking-wide uppercase ${
                      j.sorot ? "bg-[#6B4226] text-[#F3EDE0]" : "bg-[#3A2E22]/10 text-[#7A5C3E]"
                    }`}
                  >
                    {j.catatan}
                  </span>
